...
from db.dbinfo import User, DBSession
...
class Admin:
    ...
    def findByUsername(self, username):
        ...
        if result is not None:
            return {'id': result.id, 'username': result.username, 'role': result.role, 'loginTime': result.loginTime}
        else:
            return None
    def findByUsernameAndPassword(self, data):
        data['password'] = md5.str2md5(data['password'] + data['username'])
        result = self.session.query(self.User_MySQL).filter(self.User_MySQL.password == data['password']).all()
        self.session.close()
        if len(result) == 1:
            return result
        else:
...
if __name__ == '__main__':
    for i in range(0, 1):
        data = {'username': 'admin', 'password': '-pl,mko09ijnbhu', 'nickName': 'admin',
                'intro': '', 'gender': 'male', 'mobilePhone': '', 'loginTime': datetime.datetime.utcnow(),
                'status': '0',
